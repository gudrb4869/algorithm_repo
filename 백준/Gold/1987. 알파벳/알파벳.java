 ...
 * 방문여부 체크하기 위해 visited 배열 선언
 * 이미 지나온 알파벳이 있는지 확인하기 위해 isSelected 배열 선언
 * </pre>
 * @author 박형규
 *
 */
public class Main {
	...
	private static boolean[][] visited; // 보드 방문여부
	private static boolean[] isSelected = new boolean[26]; //대문자 알파벳 26개 선택여부
	...
	private static void dfs(int r, int c, int cnt) {
		visited[r][c] = true; // 현재 말이 있는 칸 방문체크
		isSelected[arr[r][c] - 'A'] = true; // 현재 말이 있는 칸에 있는 알파벳 체크
		...
		for (int i = 0; i < 4; i++) { // 4방향 체크
			...
			if (nr >= 0 && nr < R && nc >= 0 && nc < C) { // 보드범위내에 있는지
				if (!visited[nr][nc] && !isSelected[arr[nr][nc] - 'A']) {// 아직방문안했고 지나온알파벳중에 있는지 점검
					dfs(nr, nc, cnt + 1); // 다음 단계 호출
				}
			}
		}
		isSelected[arr[r][c] - 'A'] = false; // 현재 칸에 있는 알파벳 체크해제
		visited[r][c] = false; // 현재 칸 방문해제
	}
